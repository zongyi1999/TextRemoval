...
class STRnet2_change(nn.Layer):
    ...
    def __init__(self):
        ...
        self.res4 = nn.Sequential(Residual(128, 128),Residual(128, 128))
        self.res5 = Residual(128, 256, same_shape=False)
        self.res6 = Residual(256, 256)
        self.res6 = nn.Sequential(Residual(256, 256),Residual(256, 256))
        self.res7 = Residual(256, 512, same_shape=False)
        self.res8 = nn.Sequential(Residual(512, 512),Residual(512, 512))
        self.conv2 = ConvWithActivation(512, 512, kernel_size=1)
        self.deconv1 = nn.Sequential(DeConvWithActivation(512, 256, kernel_size=3,
                                            padding=1, stride=2),
                                        ConvWithActivation(256, 256, kernel_size=3,
                                              padding=1, stride=1))
        self.deconv2 =  nn.Sequential(DeConvWithActivation(256 * 2, 128, kernel_size=3,
                                            padding=1, stride=2),
                                       ConvWithActivation(128, 128, kernel_size=3,
                                              padding=1, stride=1))
        self.deconv3 = nn.Sequential(DeConvWithActivation(128 * 2, 64, kernel_size=3,
                                            padding=1, stride=2),
                                      ConvWithActivation(64, 64, kernel_size=3,
                                              padding=1, stride=1))
        self.deconv4 = nn.Sequential(DeConvWithActivation(64 * 2, 32, kernel_size=3,
                                            padding=1, stride=2),
                                      ConvWithActivation(32, 32, kernel_size=3,
                                              padding=1, stride=1))
        self.deconv5 = nn.Sequential(DeConvWithActivation(64, 3, kernel_size=3, padding=1,
                                            stride=2),
                                      ConvWithActivation(3, 3, kernel_size=3,
                                              padding=1, stride=1))
